...
void WriteKernelInfomation( int iTargetFd,int iTotalSectorCount, int iKernelSectorCount );
...
void WriteKernelInfomation(int iTargetFd,int iTotalSectorCount, int iKernelSectorCount)
{
    ...
    if( lPosition == -1 )
    {
        fprintf( stderr, "lseek fail. Return value = %ld, errno = %ld, %ld\n", 
            lPosition, (long int)errno, (long int)SEEK_SET );
        exit( -1 );
    }
