...
typedef struct KeyboardManageStruct
{
	unsigned char shiftDown;
	unsigned char capsLockOn;
	unsigned char numLockOn;
	unsigned char scrollLockOn;
	
	unsigned char extendedCodeIn;
	int skipCountForPause;
} keyboardManager;

unsigned char isOutputBufferFull();
unsigned char isInputBufferFull();
unsigned char activateKeyboard();
